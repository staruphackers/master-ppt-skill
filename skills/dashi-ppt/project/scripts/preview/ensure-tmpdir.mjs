...
//
// 事故一(0.1.25):宿主在自己的沙箱裡啟動常駐預覽服務,程序繼承了指向宿主沙箱臨時
...
//
// 事故二(0.1.27):宿主的 macOS seatbelt 沙箱會被 daemonize 的服務程序繼承。程序
...
// 兜底目錄,如匯出目錄),並把它顯式設定為瀏覽器子程序的 TMPDIR,讓 Chrome 的內部
// 臨時路徑一併改道到可寫位置。
//
// 不重建宿主的沙箱目錄——那隨時會再被清理。
...
// 返回一個實測可寫的臨時目錄,供瀏覽器子程序用作 TMPDIR。extraFallbacks 裡傳服務
  ...
  for (const candidate of candidates) {
    ...
    if (probeWritable(candidate)) {
      if (candidate !== candidates[0]) {
        ...
        // 同時修正本程序的 TMPDIR:Playwright 在 node 側也會 mkdtemp(playwright-artifacts),
        // 只給瀏覽器子程序改道不夠,宿主程序自己的 os.tmpdir() 消費方也必須落到可寫位置。
